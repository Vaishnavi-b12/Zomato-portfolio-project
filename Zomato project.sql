...
(select  top 1 product_id from sales group by product_id order by count(product_id)desc)
 ...
 5.Which item was the most popular for each customer?
   ...
   8. What is the totl orders and amount spent for each member before they became a member?
  ...
  ( select c.*,d.price from
   ...
   9. If buying each product generates points for eg5rs=2 zomato point and each product has different purchasing points for eg for p1 5rs=1 zomato point, for p2 10rs=5 zomato point and p3 5rs=1 zomato point,
   calculate points collected  by each customers and for which product most points have been given till now?

  
  select userid,sum(total_point)*2.5 total_point_earned from
  ...
  (select d.*,case when product_id=1 then 5 when product_id=2 then 2 when product_id=3 then 5 else 0 end as point from
   ...
   10. In the first one year after a customer joins the gold program(including their join date) irrespective of what the customer has purchased they earn 5 zomato points for every 10rs spent who earned more1 or 3 and what was their points earnings in their first year?


  select c.*,d.price*0.5 total_points_earned from
  (select a.userid,a.created_date,a.product_id,b.gold_signup_date from sales a inner join
   goldusers_signup b on a.userid=b.userid and created_date<=gold_signup_date and created_date<=DATEADD(year,1,gold_signup_date))c inner join
